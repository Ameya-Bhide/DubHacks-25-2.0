...
export default function Home() {
  ...
  const loadDocuments = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setFilteredDocuments(result.files) // Initialize filtered documents
  ...
  const filterDocuments = () => {
    ...
    if (documentFilters.studyGroup) {
      filtered = filtered.filter(doc => doc.studyGroupName === documentFilters.studyGroup)
    }

    // Filter by class name
    if (documentFilters.className) {
      filtered = filtered.filter(doc => doc.className === documentFilters.className)
    }

    // Filter by date range
    if (documentFilters.dateFrom) {
      const fromDate = new Date(documentFilters.dateFrom)
      filtered = filtered.filter(doc => {
        const docDate = new Date(doc.dateCreated || doc.date || doc.createdAt)
        return docDate >= fromDate
      })
    }

    if (documentFilters.dateTo) {
      const toDate = new Date(documentFilters.dateTo)
      filtered = filtered.filter(doc => {
        const docDate = new Date(doc.dateCreated || doc.date || doc.createdAt)
  ...
  const [showStudyGroupSelectionModal, setShowStudyGroupSelectionModal] = useState(false)
  const [availableStudyGroups, setAvailableStudyGroups] = useState<string[]>([])
  ...
  const [showFlashcardDeck, setShowFlashcardDeck] = useState<boolean>(false)

  // Debug flashcard deck state changes
  useEffect(() => {
    console.log('🎴 Flashcard deck state changed:', showFlashcardDeck)
  }, [showFlashcardDeck])
    ...
    try {
      ...
      const filteredDocs = documents.filter(doc => {
        ...
        return (
          ...
          doc.studyGroupName?.toLowerCase().includes(searchTerm) ||
          doc.className?.toLowerCase().includes(searchTerm)
        )
      })
  ...
  const loadStudyGroupsFromFiles = async () => {
    try {
      // Get unique study groups from documents
      const studyGroups = Array.from(new Set(documents.map(doc => doc.studyGroupName).filter(Boolean)))
      setAvailableStudyGroups(studyGroups)
      console.log('📚 Available study groups from files:', studyGroups)
    } catch (error) {
      console.error('Error loading study groups from files:', error)
  ...
  const handleAIToolClick = (toolName: string) => {
    ...
    setAiQuery('')
    loadStudyGroupsFromFiles()
    setShowStudyGroupSelectionModal(true)
  }

  const filterDocumentsByRelevance = (documents: any[], query: string, tool: string) => {
    if (!query || query.trim() === '') {
      // If no query, return all documents but prioritize PDFs
      return documents.sort((a, b) => {
        const aIsPDF = (a.fileName?.toLowerCase().endsWith('.pdf') || a.filePath?.toLowerCase().endsWith('.pdf'))
        const bIsPDF = (b.fileName?.toLowerCase().endsWith('.pdf') || b.filePath?.toLowerCase().endsWith('.pdf'))
    ...
    // Keywords that indicate relevance for different tools
    const toolKeywords: { [key: string]: string[] } = {
      'flashcards': ['flashcard', 'card', 'question', 'answer', 'quiz', 'test', 'review'],
      'summaries': ['summary', 'overview', 'notes', 'lecture', 'chapter', 'section'],
      'practice-questions': ['question', 'problem', 'exercise', 'practice', 'homework', 'assignment'],
      'search': ['search', 'find', 'look', 'locate']
    }
    
    const relevantKeywords = toolKeywords[toolLower] || []
    
    const scoredDocs = documents
      .map(doc => {
        const fileName = (doc.fileName || doc.originalFileName || '').toLowerCase()
        const description = (doc.description || '').toLowerCase()
        const studyGroupName = (doc.studyGroupName || '').toLowerCase()
        const className = (doc.className || '').toLowerCase()
        
        let relevanceScore = 1 // Base score for all documents
        
        // Check for query terms in filename
        if (fileName.includes(queryLower)) relevanceScore += 10
        if (description.includes(queryLower)) relevanceScore += 8
        if (studyGroupName.includes(queryLower)) relevanceScore += 6
        if (className.includes(queryLower)) relevanceScore += 4
        
        // Check for partial matches (substrings)
        const queryWords = queryLower.split(' ').filter(word => word.length > 2)
        queryWords.forEach(word => {
          if (fileName.includes(word)) relevanceScore += 3
          if (description.includes(word)) relevanceScore += 2
          if (studyGroupName.includes(word)) relevanceScore += 1
        })
        
        // Check for tool-specific keywords
        relevantKeywords.forEach((keyword: string) => {
          if (fileName.includes(keyword)) relevanceScore += 3
          if (description.includes(keyword)) relevanceScore += 2
        })
        
        // Boost PDF files
        const isPDF = fileName.endsWith('.pdf') || (doc.filePath && doc.filePath.toLowerCase().endsWith('.pdf'))
        if (isPDF) relevanceScore += 5
        
        // Penalize obviously irrelevant files
        if (fileName.includes('downloaded from') || fileName.includes('asdf') || description.includes('asdf')) {
          relevanceScore -= 20
        }
    ...
    const filteredDocs = scoredDocs
      .filter(item => {
        ...
        console.log('🔍 Document relevance check:', {
          fileName: item.doc.fileName || item.doc.originalFileName,
          relevanceScore: item.relevanceScore,
          passed: item.relevanceScore >= -5 // More lenient threshold
        })
        return item.relevanceScore >= -5 // More lenient threshold - allow documents with slight penalties
      })
      .sort((a, b) => b.relevanceScore - a.relevanceScore) // Sort by relevance
      .map(item => item.doc)
      .slice(0, 5) // Limit to top 5 most relevant documents
    
    // Fallback: if no documents pass the filter, return all documents (prioritizing PDFs)
    if (filteredDocs.length === 0) {
      console.log('⚠️ No documents passed relevance filter, using fallback - returning all documents')
      return documents.sort((a, b) => {
        const aIsPDF = (a.fileName?.toLowerCase().endsWith('.pdf') || a.filePath?.toLowerCase().endsWith('.pdf'))
        const bIsPDF = (b.fileName?.toLowerCase().endsWith('.pdf') || b.filePath?.toLowerCase().endsWith('.pdf'))
        if (aIsPDF && !bIsPDF) return -1
  ...
  const generateAIContent = async () => {
    if (selectedStudyGroups.length === 0) {
    ...
    setAiLoading(true)
    try {
      // Get documents from selected study groups
      const allSelectedDocuments = documents.filter(doc => 
        selectedStudyGroups.includes(doc.studyGroupName)
      )

      if (allSelectedDocuments.length === 0) {
        alert('No documents found in the selected study groups')
        return
      }

      // Use all selected documents - compile everything into one comprehensive string
      const selectedDocuments = allSelectedDocuments
      
      console.log('📋 Document compilation:', {
        totalDocuments: allSelectedDocuments.length,
        query: aiQuery,
        tool: selectedAITool
      })
      ...
      // Compile all content from selected documents into one comprehensive string
      let notesContent = ''
      let pdfCount = 0
      let textCount = 0
      
      console.log('📚 Compiling content from all selected documents...')
      
      for (const doc of selectedDocuments) {
        const fileName = doc.fileName || doc.originalFileName || 'Unknown file'
        const description = doc.description || 'No description available'
        const studyGroupName = doc.studyGroupName || 'Unknown group'
        const className = doc.className || 'Unknown class'
        
        // Check if it's a PDF file (check both fileName and filePath)
        const isPDF = fileName.toLowerCase().endsWith('.pdf') || 
                     (doc.filePath && doc.filePath.toLowerCase().endsWith('.pdf'))
        
        // Check if it's a text file (check both fileName and filePath, and also check if it's a text file without extension)
        const isTextFile = fileName.toLowerCase().endsWith('.txt') || 
                          (doc.filePath && doc.filePath.toLowerCase().endsWith('.txt')) ||
                          (doc.fileType && doc.fileType.toLowerCase().includes('text')) ||
                          (doc.fileName && !doc.fileName.toLowerCase().endsWith('.pdf') && !doc.fileName.toLowerCase().endsWith('.doc') && !doc.fileName.toLowerCase().endsWith('.docx'))
        
        console.log('🔍 Processing document:', { fileName, isPDF, isTextFile, studyGroupName, className })
          ...
          try {
            ...
            const response = await fetch('/api/extract-pdf-text', {
              ...
              body: JSON.stringify({
                ...
                maxPages: 10, // Increased to get more content
                maxChars: 5000 // Increased character limit
              }),
            })
            ...
            if (result.success && result.text) {
              notesContent += `=== ${fileName} (PDF from ${studyGroupName} - ${className}) ===\n${result.text}\n\n`
              pdfCount++
              console.log('✅ PDF content extracted successfully, length:', result.text.length)
            } else {
              // Fallback to description if PDF extraction fails
              notesContent += `=== ${fileName} (PDF - extraction failed, using description) ===\n${description}\n\n`
              textCount++
              console.log('⚠️ PDF extraction failed, using description')
            }
          } catch (error) {
            console.error('❌ PDF extraction error:', error)
            // Fallback to description
            notesContent += `=== ${fileName} (PDF - error, using description) ===\n${description}\n\n`
            textCount++
          }
        } else if (isTextFile && doc.filePath) {
          try {
            console.log('📄 Reading text file content from:', fileName)
            ...
            const response = await fetch('/api/extract-pdf-text', {
              ...
              body: JSON.stringify({
                filePath: doc.filePath,
                maxPages: 1, // Text files don't have pages, but we need to call the API
                maxChars: 10000 // Higher limit for text files
              }),
            })
            ...
            if (result.success && result.text) {
              notesContent += `=== ${fileName} (Text file from ${studyGroupName} - ${className}) ===\n${result.text}\n\n`
              textCount++
              console.log('✅ Text file content read successfully, length:', result.text.length)
            } else {
              // Fallback to description if text file reading fails
              notesContent += `=== ${fileName} (Text file - reading failed, using description) ===\n${description}\n\n`
              textCount++
              console.log('⚠️ Text file reading failed, using description')
            }
          } catch (error) {
            console.error('❌ Text file reading error:', error)
            // Fallback to description
            notesContent += `=== ${fileName} (Text file - error, using description) ===\n${description}\n\n`
            textCount++
          }
        } else {
          // For other files, use description
          notesContent += `=== ${fileName} (Other file from ${studyGroupName} - ${className}) ===\n${description}\n\n`
          textCount++
        }
      }
      
      console.log('📊 Content compilation complete:', {
        totalDocuments: selectedDocuments.length,
        pdfsProcessed: pdfCount,
        textFilesProcessed: textCount,
        totalContentLength: notesContent.length
      })

      console.log('📄 Final content for AI (length):', notesContent.length)

      // Map AI tool names to API actions
      ...
      const action = actionMap[selectedAITool]
      if (!action) {
        throw new Error(`Unknown AI tool: ${selectedAITool}`)
      }

      // Prepare request data with enhanced query
      const enhancedQuery = aiQuery 
        ? `${aiQuery}. Please focus on the most relevant content from the provided documents.`
        : 'Please analyze the provided content and generate appropriate material.'
      ...
      if (selectedAITool === 'flashcards') {
        requestData.numCards = 10
      } else if (selectedAITool === 'practice-questions') {
        requestData.numQuestions = 5
      } else if (selectedAITool === 'search') {
        // For search, we need to provide YAML content
        const yamlContent = selectedDocuments
          .map(doc => `"${doc.filePath || doc.s3Key}":\n  one_sentence: "${doc.description || doc.fileName}"`)
          .join('\n')
        requestData.yamlContent = yamlContent
        requestData.prompt = aiQuery || 'Find relevant content'
      }
      ...
      // Set the result and show appropriate modal
      const aiContent = result.reply || result.results || 'No result generated'
      console.log('📝 AI Content extracted:', aiContent.substring(0, 200) + '...')
      setAiResult(aiContent)
      
      // Show flashcard deck for flashcards, regular modal for others
      console.log('🔍 AI Tool Decision:', { selectedAITool, aiContent: aiContent.substring(0, 100) + '...' })
      if (selectedAITool === 'flashcards') {
        console.log('🎴 Showing flashcard deck')
        console.log('🎴 Setting showFlashcardDeck to true')
        setShowFlashcardDeck(true)
        console.log('🎴 showFlashcardDeck state should now be true')
      } else {
        console.log('📄 Showing regular AI result modal')
        setShowAIResultModal(true)
      }

      // Close the study group selection modal and reset state
      setShowStudyGroupSelectionModal(false)
      setSelectedStudyGroups([])
      setSelectedAITool('')
      setAiQuery('')
      ...
      alert(`Failed to generate ${selectedAITool}: ${error.message}`)
    } finally {
      setAiLoading(false)
    }
  }
  ...
  const handleDocumentDelete = async (fileId: string, filePath: string) => {
    try {
      ...
      if (result.success) {
        ...
        setFilteredDocuments(prev => prev.filter(doc => doc.id !== fileId))
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        {activeTab === 'ai' && (
          <div className="bg-white rounded-xl shadow-sm p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {/* Flashcards Button */}
              <button
                onClick={() => handleAIToolClick('flashcards')}
              ...
              {/* Summaries Button */}
              <button
                onClick={() => handleAIToolClick('summaries')}
              ...
              <button
                onClick={() => handleAIToolClick('practice-questions')}
              ...
              {/* Search Button */}
              <button
                onClick={() => handleAIToolClick('search')}
                className="bg-gradient-to-br from-orange-500 to-orange-600 text-white p-6 rounded-xl shadow-lg hover:shadow-xl transition-all duration-200 transform hover:-translate-y-1"
              >
                <div className="text-center">
                  <div className="mx-auto w-16 h-16 bg-white bg-opacity-20 rounded-full flex items-center justify-center mb-4">
                    <svg className="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <p className="text-orange-100 text-sm">Search through your documents and study materials</p>
                </div>
              </button>
            </div>
          </div>
        ...
        {activeTab === 'documents' && (
          <div className="bg-white rounded-xl shadow-sm p-8">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <select
                    value={documentFilters.studyGroup}
                    onChange={(e) => handleDocumentFilterChange('studyGroup', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="">All Study Groups</option>
                    {Array.from(new Set(documents.map(doc => doc.studyGroupName).filter(Boolean))).map(group => (
                ...
                <div>
                  ...
                  <select
                    value={documentFilters.className}
                    onChange={(e) => handleDocumentFilterChange('className', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    <option value="">All Classes</option>
                    {Array.from(new Set(documents.map(doc => doc.className).filter(Boolean))).map(className => (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleDocumentFilterChange('dateFrom', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleDocumentFilterChange('dateTo', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  />
                </div>
              </div>
              ...
              <div className="mt-4 text-sm text-gray-600">
                ...
                {Object.values(documentFilters).some(filter => filter !== '') && (
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">
                {documents.length === 0 
                  ? 'Upload and organize your study materials, notes, and resources.'
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              <h3 className="text-xl font-bold text-gray-900">Search Documents</h3>
              <button
                onClick={() => {
                ...
                <div className="flex space-x-3">
                  <div className="flex-1 relative">
                    <input
                      ...
                      placeholder="Search documents by name, description, study group, or class..."
                  ...
                  <button
                    ...
                    className="px-6 py-3 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
                  >
                    {searchLoading ? (
                      <>
              ...
              <div className="max-h-96 overflow-y-auto">
                {searchResults.length === 0 && searchQuery ? (
                    ...
                    <div className="grid gap-4">
                      {searchResults.map((doc, index) => (
                        <div key={index} className="bg-gray-50 rounded-lg p-4 border border-gray-200 hover:shadow-md transition-shadow">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <h5 className="font-medium text-gray-900 text-sm mb-1">{doc.fileName || doc.originalFileName}</h5>
                              {doc.description && (
                            ...
                            <div className="ml-4">
                              <button
                                onClick={() => handleDocumentOpen(doc.filePath || doc.s3Key, doc.fileName || doc.originalFileName)}
                                className="px-3 py-1 bg-orange-600 text-white text-xs rounded hover:bg-orange-700 transition duration-200"
                              >
                                Open
                              </button>
                            </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <h4 className="text-lg font-medium text-gray-900 mb-2">Search Your Documents</h4>
                    <p className="text-gray-600">Enter keywords to search through your uploaded documents and study materials.</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {/* Study Group Selection Modal */}
      {showStudyGroupSelectionModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[80vh] overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              <h3 className="text-xl font-bold text-gray-900">
                Select Study Groups for {selectedAITool.charAt(0).toUpperCase() + selectedAITool.slice(1).replace('-', ' ')}
              </h3>
              <button
                onClick={() => {
                  setShowStudyGroupSelectionModal(false)
            ...
            <div className="p-6">
              ...
              <div className="mb-6">
                <p className="text-gray-600 text-sm">
                  Select one or more study groups to generate {selectedAITool.replace('-', ' ')} from their documents.
                </p>
              </div>
              ...
              <div className="mb-6">
                ...
                <textarea
                  ...
                  placeholder={`Enter a specific query for ${selectedAITool.replace('-', ' ')} generation (e.g., "focus on key concepts", "include examples", "difficulty level: intermediate")`}
                  rows={3}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                />
                <p className="text-xs text-gray-500 mt-1">
                  Provide specific instructions or focus areas for the AI tool to consider when processing your documents.
                </p>
              </div>
              ...
              <div className="max-h-96 overflow-y-auto mb-6">
                {availableStudyGroups.length === 0 ? (
                  <div className="text-center py-8">
                    ...
                    <h4 className="text-lg font-medium text-gray-900 mb-2">No Study Groups Found</h4>
                    <p className="text-gray-600">Upload some documents first to create study groups.</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {availableStudyGroups.map((studyGroup, index) => (
                      <div key={index} className="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                        <input
                          type="checkbox"
                          id={`study-group-${index}`}
                          checked={selectedStudyGroups.includes(studyGroup)}
                          onChange={(e) => {
                            if (e.target.checked) {
                              setSelectedStudyGroups([...selectedStudyGroups, studyGroup])
                            } else {
                              setSelectedStudyGroups(selectedStudyGroups.filter(sg => sg !== studyGroup))
                            }
                          }}
                          className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                        />
                        <label htmlFor={`study-group-${index}`} className="flex-1 cursor-pointer">
                          <div className="flex items-center justify-between">
                            <span className="text-sm font-medium text-gray-900">{studyGroup}</span>
                            <span className="text-xs text-gray-500">
                              {documents.filter(doc => doc.studyGroupName === studyGroup).length} document{documents.filter(doc => doc.studyGroupName === studyGroup).length !== 1 ? 's' : ''}
                            </span>
                          </div>
                        </label>
                      </div>
              ...
              <div className="flex justify-between items-center">
                <div className="text-sm text-gray-600">
                  {selectedStudyGroups.length > 0 && (
                    <span>{selectedStudyGroups.length} study group{selectedStudyGroups.length !== 1 ? 's' : ''} selected</span>
                  )}
                </div>
                <div className="flex space-x-3">
                  <button
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={selectedStudyGroups.length === 0 || aiLoading}
                      className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center space-x-2"
                    >
                      {aiLoading ? (
                        <>
                        ...
                        <span>Generate {selectedAITool.replace('-', ' ')}</span>
                      )}
                    </button>
                </div>
              </div>
      ...
      {showAIResultModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl max-w-4xl w-full max-h-[80vh] overflow-hidden">
            <div className="flex justify-between items-center p-6 border-b border-gray-200">
              <h3 className="text-xl font-bold text-gray-900">
                Generated {selectedAITool.charAt(0).toUpperCase() + selectedAITool.slice(1).replace('-', ' ')}
              </h3>
              <button
                onClick={() => {
